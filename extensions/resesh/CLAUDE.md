...
- **`providers/claude.ts`** — Claude Code provider. Reads `~/.claude/projects/{encoded-path}/{sessionId}.jsonl`. Also scans the default WSL distro (gated behind the `includeWsl` preference). Uses `ensureWslScan()` for deduplicated scanning with in-memory cache. Exports `claudeProvider`.
- **`providers/wsl.ts`** — Shared WSL infrastructure used by both providers. `getDefaultWslDistro()` (finds the default distro via `wsl -l`, cached as a shared promise), `wslEnabled()` (checks `includeWsl` preference), `execWsl()` (runs a shell script inside WSL, returns stdout), `streamWslFiles()` (streams JSONL file contents via `\tFILE\t` header protocol, supports `AbortSignal` to kill the process on cancel).
- **`providers/codex.ts`** — Codex CLI provider. Reads `~/.codex/sessions/YYYY/MM/DD/rollout-*.jsonl`. Also scans the default WSL distro (gated behind `includeWsl` preference). Uses `ensureWslScan()` for deduplicated scanning with in-memory cache. Exports `codexProvider`.
...
- **`actions.tsx`** — Windows terminal and IDE launch. Accepts a `resumeCommand` string prop so each provider controls its own resume CLI invocation. Supports Windows Terminal (`wt.exe`), PowerShell, Command Prompt, and Warp (via `warp://` URI scheme + clipboard). WSL sessions use `wsl -d <distro>` for terminal launch and UNC paths for IDE/File Explorer.
